...
# Group B: Ghost SKU fix — all need SOL-CI-DT-103-FP-103
ghost_orders = [
    ('SOL1203900', 'SHP27-10205', 'co671ul941'),   # Pujitha Thota, 502032 TG, Prepaid
    ('SOL1203828', 'SHP27-10135', 'dshbbt5kgo'),   # Jolene, 403705 Goa, Prepaid
    ('SOL1203782', 'SHP27-10087', 'fqgq5ovo2l'),   # Anu Santhosh, 682315 KL, Prepaid
    ('SOL1203717', 'SHP27-10022', 'c9hd7vvs0o'),   # Shailendra yadav, 284003 UP, Prepaid
    ('SOL1203909', 'SHP27-10214', '23d19aa4ij'),   # ullas babuv, 689112 KL, PPCOD COD=1799.1
]

sku = 'SOL-CI-DT-103-FP-103'
results = []

# Step 1: Fix all ghost SKUs on SOs
print("STEP 1: Fixing ghost SKUs on SOs")
print("="*70)
for sol, so_name, child_name in ghost_orders:
    ...
    if r.status_code == 200:
        ...
        print(f'  {sol}: Script create FAIL {r.status_code}')
        results.append((sol, 'FAIL_GHOST'))
    time.sleep(0.5)
...
for sol, so_name, child_name in ghost_orders:
    ...
    # Delete any draft DNs (they have ghost SKU)
    for d in dns:
        ...
        try:
            ...
            if errs:
                ...
                for line in err.split('\n'):
                    ll = line.lower()
                    if any(k in ll for k in ['clickpost','serviceable','cod','pincode','error','fail','stock','negative','mismatch','address']):

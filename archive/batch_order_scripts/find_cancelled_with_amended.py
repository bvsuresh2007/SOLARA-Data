...
# Find orders with cancelled original + amended version that has AWB
results = []
for oid in orders:
    so_list = all_sos.get(oid, [])
    cancelled = [so for so in so_list if so["docstatus"] == 2]
    submitted = [so for so in so_list if so["docstatus"] == 1]

    if not cancelled or not submitted:
        continue

    # Check if submitted version has DN with AWB
    for so in submitted:
        r = s.get(f"{BASE}/api/resource/Delivery Note", params={
            ...
            "fields": json.dumps(["name","awb_number","docstatus"]),
            "limit_page_length": 5
        }, timeout=30)
        dns = r.json().get("data", [])
        awb = ""
        ...
        for dn in dns:
            if dn.get("awb_number"):
                ...
                dn_name = dn["name"]
                break
        if not dn_name and dns:
            dn_name = dns[0]["name"]

        results.append({
            "oid": oid,
            "cancelled_so": cancelled[0]["name"],
            "amended_so": so["name"],
            "dn": dn_name,
            "awb": awb,
        })
...
print(f"Orders with cancelled original + amended version: {len(results)}\n")
for i, r in enumerate(results, 1):
    icon = "AWB" if r["awb"] else "NO AWB"
    print(f'{i}. {r["oid"]} | Cancelled: {r["cancelled_so"]} | Amended: {r["amended_so"]} | DN: {r["dn"]} | AWB: {r["awb"] or "-"} [{icon}]')

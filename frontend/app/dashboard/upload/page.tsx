...
export default function UploadPage() {
  ...
  // Load supported file types from the API
  useEffect(() => {
  ...
  async function handleUpload() {
    ...
    try {
      const res = await fetch(
        `${BASE}/api/uploads/file?file_type=${selectedType}`,
      ...
      if (res.status === 422) {
        // Column mismatch — file not imported
        setParseError(data.detail ?? data);
      } else if (res.ok) {
        setResult(data as UploadResult);
      } else {
        setNetworkError(`Server error ${res.status}: ${data?.detail ?? "unknown error"}`);
      }
    } catch {
    ...
    <main className="min-h-screen bg-zinc-950 p-6 space-y-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        <nav className="flex gap-4 text-sm font-medium">
          ...
          <a href="/dashboard/upload"       className="text-orange-400 border-b border-orange-400 pb-0.5">Upload</a>
        </nav>
      </header>
      ...
      <div className="max-w-2xl space-y-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {fileTypes.length === 0 && !networkError ? (
              ...
              <select
                value={selectedType}
                onChange={(e) => { setSelectedType(e.target.value); clearResult(); }}
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-zinc-100 text-sm cursor-pointer focus:outline-none focus:border-zinc-500 transition-colors"
              >
                {fileTypes.map((t) => (
              ...
              {file ? (
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="text-xs text-zinc-600 hover:text-zinc-300 underline mt-1 transition-colors"
                  >
                    Remove
                  </button>
                </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold transition-all duration-150
            ...
            disabled:opacity-40 disabled:cursor-not-allowed disabled:bg-zinc-700 disabled:text-zinc-400"
        >
          {uploading ? (
            <>
            ...
            <CardContent className="space-y-3 text-xs">
              ...
              <div className="space-y-1.5">
                <p className="text-[10px] text-zinc-600 uppercase tracking-wider">
                  Expected (missing from file)
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {parseError.missing_columns.map((c) => (
                    <span
                      key={c}
                      className="bg-red-900/40 text-red-300 border border-red-800 rounded px-2 py-0.5 font-mono text-[11px]"
                    >
                      {c}
                    </span>
                  ))}
              ...
              <div className="space-y-1.5">
                <p className="text-[10px] text-zinc-600 uppercase tracking-wider">
                  Columns found in uploaded file
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {parseError.columns_found_in_file.map((c) => (
                    <span
                      key={c}
                      className="bg-zinc-800 text-zinc-400 rounded px-2 py-0.5 font-mono text-[11px]"
                    >
                      {c}
                    </span>
                  ))}
              ...
              <p className="text-zinc-600 leading-relaxed">
                Common causes: wrong file type selected, portal changed their export format,
                or file was re-saved in Excel (altering column headers).
              </p>
            </CardContent>
          </Card>
        )}
        ...
        {result && (
          <Card className={result.inserted > 0 ? "border-green-800" : "border-zinc-700"}>
            ...
            <CardContent className="space-y-5">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                  <p className={`text-3xl font-bold ${
                    result.errors.length > 0 ? "text-yellow-400" : "text-zinc-700"
                  }`}>
                ...
                <div>
                  <div className="h-2 bg-zinc-800 rounded-full overflow-hidden flex">
                    <div
                      ...
                      style={{ width: `${(result.inserted / result.rows_parsed) * 100}%` }}
                    />
                    <div
                      className="h-full bg-zinc-600 transition-all"
                      style={{ width: `${(result.skipped / result.rows_parsed) * 100}%` }}
                    />
                    {result.errors.length > 0 && (
                      <div
                        className="h-full bg-yellow-600 transition-all"
                        style={{ width: `${(result.errors.length / result.rows_parsed) * 100}%` }}
                      />
                    )}
                  </div>
              ...
              {/* Row-level errors (unmapped SKUs etc.) */}
              {result.errors.length > 0 && (
                <div>
                  ...
                  <div className="max-h-48 overflow-y-auto rounded-lg bg-zinc-800/50 p-3 space-y-1.5">
                    {result.errors.slice(0, 50).map((e, i) => (
                      <div key={i} className="flex gap-3 text-xs">
                        <span className="text-zinc-600 w-14 flex-shrink-0 font-mono">
                          row {e.row}
                        </span>
              ...
              {result.import_log_id && (
                <p className="text-[10px] text-zinc-600">
                  Audit log ID: {result.import_log_id}
                </p>
              )}
            </CardContent>
          </Card>

...
def run_server_script(name, script):
    """Create, wait for cache clear, run, delete."""
    ...
    # Clear cache to ensure script is registered
    ...
    for attempt in range(3):
        ...
        if 'Failed to get method' in str(exc):
            print(f'  Attempt {attempt+1}: Script not registered yet, waiting...')
            time.sleep(3)
            continue
        if exc:
    ...
    print(f'  Script never registered after 3 attempts')
    requests.delete(f'{BASE}/api/resource/Server Script/{name}', headers=H, timeout=10)
    return None
...
msg1 = run_server_script('tmp_cso_435d', script1)
print(f'  Result: {msg1}')
...
msg2 = run_server_script('tmp_sso_435d', script2)
print(f'  Result: {msg2}')
